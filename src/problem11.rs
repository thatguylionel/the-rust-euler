...
#[allow(dead_code)]
pub fn execute() {
    // make row by row into a vector of vectors of i32
    let combined = vec![
...
fn vertical(combined: Vec<Vec<i32>>) -> (i32, i32) {
    ...
    // build a new grid where each horizontal row is a vertical row in the new grid
    loop{
        let mut current_position = 0;
        for row in combined.iter() {
            if current_position == 20 { break; }
            new_row.push(row[current_row]);
            current_position += 1;
        }
        new_list.push(new_row.clone());
        new_row.clear();
        current_row += 1;
    ...
    return (winning_row as i32, high_score);
}

fn horizontal(combined: Vec<Vec<i32>>) -> (i32, i32){
    let mut high_score = 0;
    let mut winning_row = 0;
    let mut current_row = 0;
    for row in combined.iter() {
    ...
    return (winning_row, high_score);
}

fn get_result(current_row: Vec<i32>, current_row_rev: Vec<i32>) -> i32 {
    let mut list: Vec<Vec<i32>> = Vec::new();
    list.push(current_row.clone());
    list.push(current_row_rev);
    let mut high_score = 0;
    for row in list.iter() {
        let mut current_position = 0;
        for _ in current_row.iter() {
            if current_position == 17 { break; }
            let mut sum = 1;
            for item in current_row[current_position..current_position + 4].iter() {
                sum *= item;
            }
            if sum > high_score {
                high_score = sum;
